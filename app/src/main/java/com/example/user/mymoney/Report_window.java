...
public class Report_window extends AppCompatActivity implements View.OnClickListener{
    ...
    public void onClick(View v)
    {
        ...
        DBHelper_Incomes dbHelper_incomes = new DBHelper_Incomes(this);
        SQLiteDatabase database = dbHelper_incomes.getReadableDatabase();
        Cursor cursor = database.query(DBHelper_Incomes.TABLE_INCOMES, null, null, null, null, null, null);
        if (cursor.moveToFirst()) {
            int idIndex = cursor.getColumnIndex(DBHelper_Incomes.KEY_ID);
            int incomeIndex = cursor.getColumnIndex(DBHelper_Incomes.KEY_INCOME);
            int categoryIndex = cursor.getColumnIndex(DBHelper_Incomes.KEY_CATEGORY);
            int descriptionIndex = cursor.getColumnIndex(DBHelper_Incomes.KEY_DESCRIPTION);
            int dayIndex = cursor.getColumnIndex(DBHelper_Incomes.KEY_DAY);
            int monthIndex = cursor.getColumnIndex(DBHelper_Incomes.KEY_MONTH);
            int yearIndex = cursor.getColumnIndex(DBHelper_Incomes.KEY_YEAR);

            int day_count=0, month_count=0, year_count=0;
            do {
                if(selected_date.equals("За день")) {
                    if (str_current_day.equals(cursor.getString(dayIndex)) && str_current_month.equals(cursor.getString(monthIndex)) && str_current_year.equals(cursor.getString(yearIndex))) {
                        ...
                        {
                            tvReport.append("" + cursor.getString(categoryIndex) + " " + cursor.getString(incomeIndex) +
                                    " " + cursor.getString(dayIndex) + "/" + cursor.getString(monthIndex) + "/" + cursor.getString(yearIndex));
                            tvReport.append("\n");
                        }
                     day_count++;
